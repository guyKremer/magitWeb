...
public class Commit implements CommitRepresentative {
    ...
    String m_message;
    Folder m_rootFolder;
    List<String> m_prevCommitSha1Array = new ArrayList<String>();
    String m_dateOfCreation;
    String m_creator;
    ...
    public Commit(Commit currCommit) {
        m_repositoryPath = currCommit.m_repositoryPath;
        m_pathToMagitDirectory = currCommit.m_pathToMagitDirectory;
        m_message = currCommit.m_message;
        m_rootFolder = currCommit.m_rootFolder;
        m_prevCommitSha1Array = currCommit.m_prevCommitSha1Array;
        m_dateOfCreation = currCommit.m_dateOfCreation;
        m_creator = currCommit.m_creator;
        createSha1();
    }
    ...
    public void readPartialCommitLines(List<String> i_lines) {
        ...
        for (int i = 0; i < i_lines.size(); i++) {
            switch (i) {
                case 1:
                    setFirstPrecedingSha1(i_lines.get(i));
                    break;
                case 2:
                    setSecondPrecedingSha1(i_lines.get(i));
                    break;

                case 3:
                    setM_message(i_lines.get(i));
                    break;

                case 4:
                    setM_dateOfCreation(i_lines.get(i));
                    break;

                case 5:
                    setM_creator(i_lines.get(i));
            }
        }
